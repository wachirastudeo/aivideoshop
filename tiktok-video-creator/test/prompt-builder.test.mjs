...
check("image prompt with woman presenter changes intro layout text", /โดยมีผู้พรีเซนต์ร่วมแสดงสินค้าอยู่ในมุมมองต่าง ๆ/i.test(imgPresenterWoman), imgPresenterWoman);
...
check("image prompt with no presenter maintains grid layout text", /ที่แสดงรายละเอียดสินค้าจากหลากหลายมุมมอง/i.test(imgPresenterNone), imgPresenterNone);
...
check("image prompt with hands_only presenter uses hands intro", /โดยมีมือคนร่วมถือหรือแสดงสินค้าอยู่ในมุมมองต่าง ๆ/i.test(imgPresenterHands), imgPresenterHands);
