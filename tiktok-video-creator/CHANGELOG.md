...
- Fixed **empty caption/hashtag errors and unsynced React state** on TikTok Studio. Added a strict safety fallback in `video-output.js`: if the AI-generated caption or edited caption is ever empty, null, or whitespace-only, the script will automatically fallback to the default template-based caption. Also wrapped the `caption` variable in `String(caption || "").trim()` inside `fillCaptionAndHashtags` and `fillCaption` in `tiktok-studio-automation.js` to prevent JavaScript crashes when performing regex operations on empty or null values. Fixed programmatic typing issues in Draft.js by implementing **humanTypeWords** (simulated human word-by-word typing logic with random pauses) exactly like the Google Flow page, which naturally and reliably triggers all Draft.js React state updates. Completely resolved the caption disappearance bug on Post click by removing selection-clearing blur/focus checking loops and eliminating the trailing `input` event space payload (`data: " "`) which was mistakenly overwriting the typed text with a single space.
