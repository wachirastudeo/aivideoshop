...
async function analyzeWithGemini(imageDataUrls, productInfo, settings) {
  ...
  const prompt = [
    ...
    "For promptAdvice, write concise English guidance that preserves only the named product. Explicitly instruct generation to discard the original background and unrelated objects, including surrounding/overlay text, choose a new setting suitable for the product category, preserve the exact color palette, design details, patterns, and style from the reference image, and ensure only text physically printed on the product or its packaging is rendered extremely sharp, clear, legible, and spelt correctly in both Thai and English.",
...
function normalizeAutoOptions(value, productInfo = {}) {
  ...
  return {
    videoStyle: pickAllowed(raw.videoStyle, ["sales", "review", "lifestyle", "flash-sale", "unboxing", "before-after", "testimonial", "cinematic", "still-motion", "boxed-motion", "trending-hook"], inferred.videoStyle),
    presenter,
