...
async function analyzeWithGemini(imageDataUrls, productInfo, settings) {
  ...
  const prompt = [
    ...
    "For hooks, generate an array of 5 distinct high-converting sales hooks in Thai, customized for TikTok Shop. Each hook must be under 50 characters, punchy, stop the scroll, and appeal to different angles (e.g. pain points, promotions/discounts, transformation/results, trending/social proof, urgency). Crucially, the tone must be natural, realistic, conversational, and UGC-style, avoiding overly exaggerated, clickbaity claims, or cringey buzzwords (e.g. do NOT use words like 'กรี๊ด', 'กราบ', 'เกินต้าน', 'ปังไม่ไหว', 'อึ้ง!', 'สั่นสะเทือนวงการ!', 'ที่สุดในชีวิต!', 'ต้องลองด่วนที่สุด!'). Keep them relatable, pleasant, and authentic.",
